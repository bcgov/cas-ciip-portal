        ...
        $function$
            begin
                if search_field is null or search_value is null
                    then return query execute 'with x as (select a.id as application_id, o.operator_name, f.facility_name, a.reporting_year, f.bcghgid, status.created_at as submission_date, status.application_revision_status
                    ...
                    return query execute 'with x as (select a.id as application_id, o.operator_name, f.facility_name, a.reporting_year, f.bcghgid, status.created_at as submission_date, status.application_revision_status
                                        from ggircs_portal.application a
                                        join ggircs_portal.application_revision ar on a.id = ar.application_id
                                        join ggircs_portal.application_revision_status status on ar.application_id = status.application_id
                                          and ar.version_number = status.version_number
