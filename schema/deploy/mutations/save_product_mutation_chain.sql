...
new_state varchar(1000), new_parent int ARRAY, new_requires_emission_allocation boolean)
...
begin
  ...
  insert into ggircs_portal.product(name, description, units, state, parent, requires_emission_allocation)
  values (new_name, new_description, new_units, new_state, new_parent, new_requires_emission_allocation) returning id into new_id;
  ...
  select id, name, description, state, parent, requires_emission_allocation from ggircs_portal.product where id = new_id into result;
  return result;
end;
