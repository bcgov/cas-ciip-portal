...
create trigger _100_timestamps
  before insert or update on ggircs_portal.product_link
  for each row
...
perform ggircs_portal_private.grant_permissions('select', 'product_link', 'ciip_analyst');

-- Grant ciip_industry_user permissions
perform ggircs_portal_private.grant_permissions('select', 'product_link', 'ciip_industry_user');
...
comment on table ggircs_portal.product_link is 'Table containing the information for the links between CIIP products. A link from product_id -> linked_product_id requires linked_product_id to be reported if product_id is reported';
...
comment on column ggircs_portal.product_link.updated_by is 'Updator of row';
comment on column ggircs_portal.product_link.deleted_at is 'Date of deletion';
comment on column ggircs_portal.product_link.deleted_by is 'The user who deleted the row';
