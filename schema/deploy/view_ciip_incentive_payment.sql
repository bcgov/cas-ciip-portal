...
create or replace view ggircs_portal.ciip_incentive_payment as (
  ...
  with ciip_non_bio_emission as (
    select sum(annual_co2e) as annual_co2e, ciip_emission.application_id
  ...
  energy_balance as (
    ...
    join ggircs_portal.ciip_electricity_and_heat elec on
      elec.application_id = application.id and elec.energy_type = 'electricity'
    join ggircs_portal.ciip_electricity_and_heat heat on
      heat.application_id = application.id and heat.energy_type = 'heat'
  ),
  facility_carbon_tax as (
    select application_id, sum(calculated_carbon_tax) as calculated_carbon_tax
    from ggircs_portal.ciip_carbon_tax_calculation
    group by application_id
  ),
  product_efficiency as (
    select ciip_production.application_id, ciip_production.product_id,
  ...
  select ciip_production.application_id, ciip_production.product_id, product_efficiency.production_efficiency,
  ((
  ...
  ) * benchmark.incentive_multiplier * (ciip_production.payment_allocation_factor/100) * facility_carbon_tax.calculated_carbon_tax) as incentive_amount
  from ggircs_portal.ciip_production
  join facility_carbon_tax on ciip_production.application_id = facility_carbon_tax.application_id
