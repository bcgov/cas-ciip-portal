        ...
        $function$
            begin
                     if search_field is null or search_value is null
                        ...
                        and p.state != ''deprecated''),
                        tempTable as (select id, name, description, units, state, parent, created_at, created_by, updated_at, updated_by, deleted_at, deleted_by from outerTable
                        where (CURRENT_TIMESTAMP between start_date and end_date or (CURRENT_TIMESTAMP > start_date and end_date is null) or benchmark is null)
                    ...
                    'with outerTable as (select p.*, benchmark, eligibility_threshold, start_date, end_date from ggircs_portal.product as p left join ggircs_portal.benchmark as b on p.id = b.product_id
                        and p.state != ''deprecated''),
                        tempTable as (select id, name, description, units, state, parent, created_at, created_by, updated_at, updated_by, deleted_at, deleted_by from outerTable
                        where '|| search_field || '::text ilike ''%' || search_value || '%'' and state != ''deprecated''
