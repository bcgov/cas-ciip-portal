...
begin
  ...
  case
    when add_create = true then
      column_string := concat('alter table ', table_name, ' add column if not exists created_by int references ggircs_portal.ciip_user(id), add column if not exists created_at timestamptz not null default now()');
    ...
    when add_update = true then
      column_string := concat('alter table ', table_name, ' add column if not exists updated_by int references ggircs_portal.ciip_user(id), add column if not exists updated_at timestamptz not null default now()');
      execute(column_string);
      ...
      execute(index_string);
    when add_delete = true then
      column_string := concat('alter table ', table_name, ' add column if not exists deleted_by int references ggircs_portal.ciip_user(id), add column if not exists deleted_at timestamptz');
      execute(column_string);
