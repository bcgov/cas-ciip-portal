...
set jwt.claims.sub to 'business@provider';
set jwt.claims.email to 'bob.loblaw@gov.bc.ca';
...
select results_eq (
  $$
    select first_name, last_name, email_address, allow_uuid_update, bceid_business_name
    from ggircs_portal.ciip_user
    where uuid = 'business@provider'::varchar
  $$,
  $$
  values (
    'Bob'::varchar(1000),
  ...
  $$,
  'create_or_update_ciip_user successfully adds a new user if the email matches a basic bceid, but bceid_business_name is not null'
);

set jwt.claims.sub to 'different_business@provider';
set jwt.claims.email to 'bob.loblaw@gov.bc.ca';
set jwt.claims.bceid_business_name to 'different_business';
...
select results_eq (
  $$
    select first_name, last_name, email_address, allow_uuid_update, bceid_business_name
    from ggircs_portal.ciip_user
    where uuid = 'different_business@provider'::varchar
  $$,
  $$
  values (
    'Bob'::varchar(1000),
    ...
    'bob.loblaw@gov.bc.ca'::varchar(1000),
    false,
    'different_business'::varchar(1000)
  )
  $$,
  'create_or_update_ciip_user successfully adds a new user if the email matches a business bceid, but bceid_business_name is different from the existing row'
);

update ggircs_portal.ciip_user set allow_uuid_update = true where uuid='business@provider'::varchar;

set jwt.claims.sub to 'new_business@provider';
set jwt.claims.email to 'bob.loblaw@gov.bc.ca';
set jwt.claims.bceid_business_name to 'business';
...
select results_eq (
  $$
    select uuid, first_name, last_name, email_address, allow_uuid_update, bceid_business_name
    from ggircs_portal.ciip_user
    where uuid = 'new_business@provider'::varchar
  $$,
  $$
  values (
    'new_business@provider'::varchar(1000),
    'Bob'::varchar(1000),
    'Loblaw'::varchar(1000),
    'bob.loblaw@gov.bc.ca'::varchar(1000),
    false,
  ...
  $$,
  'create_or_update_ciip_user successfully updates the uuid of a business bceid user if the allow_uuid_update flag is set'
);

update ggircs_portal.ciip_user set allow_uuid_update = false;

-- Throws if we try to change the sub on a user that doesn't have the flag set and the bceid_business_name is the same
set jwt.claims.sub to 'changed@new_provider';
set jwt.claims.email to 'bob.loblaw@gov.bc.ca';
set jwt.claims.bceid_business_name to 'business';
...
select throws_like (
  ...
  $$,
  'uuid cannot be updated when allow_uuid_update is false',
  'throws if the user has a new sub for an existing email address and bceid_business_name, and the allow_uuid_update flag is set to false'
);
