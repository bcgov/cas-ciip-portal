...
select results_eq(
  ...
  $$,
  array[1::integer],
  'Custom mutation creates a row if the product_naics_code code does not already exist'
);
...
select results_eq(
  $$
    select is_mandatory from ggircs_portal.product_naics_code where product_id=1;
  $$,
  array['true'::boolean],
...
select is_empty(
  $$
    select *  from ggircs_portal.product_naics_code where deleted_at is not null;
  $$,
  'Custom mutation sets deleted_at to null when the product_naics_code_code already exists'
);
