...
prepare prepared_enum_test (text) AS select id from ggircs_portal.product where product_state = $1;

select lives_ok( 'execute prepared_enum_test(''published'')', 'should not complain that the operator does not exist');
