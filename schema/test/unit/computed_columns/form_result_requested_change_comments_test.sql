...
select has_function(
  'ggircs_portal', 'form_result_requested_change_comments', array['ggircs_portal.form_result'],
...
select set_eq(
  $$
    ...
    select description from ggircs_portal.form_result_requested_change_comments((select * from record));
  $$,
  ARRAY['change'::varchar(100000)],
  'form_result_requested_change_comments returns requested change type comments'
);

select is_empty(
  $$
    with record as (select row( form_result.*)::ggircs_portal.form_result from ggircs_portal.form_result where application_id = (select max(id) from ggircs_portal.application) and version_number = 1 and form_id = 1)
    select * from ggircs_portal.form_result_requested_change_comments((select * from record)) where comment_type != 'requested change'::ggircs_portal.review_comment_type;
  $$,
  'form_result_requested_change_comments does not return requested change type comments'
);
