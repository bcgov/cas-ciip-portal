...
select has_function(
  'ggircs_portal', 'application_revision_emission_form_data',
  'Function application_revision_emission_form_data should exist'
);
...
select results_eq(
  $$
    ...
    select annual_co2e from ggircs_portal.application_revision_emission_form_data((select * from record))
  $$,
  $$
    select annual_co2e from ggircs_portal.ciip_emission where application_id=1 and version_number=1;
  $$,
  'application_revision_emission_form_data retrieves the emission data for a specific application revision'
);
