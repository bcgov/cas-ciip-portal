...
select has_function(
  ...
  'Function init_application_administration_form_result should exist'
);

update swrs.facility
set facility_name = E'string\nwith\nnewlines'
where report_id = (
  select report_id from ggircs_portal.facility where id = 1
);

select lives_ok($$
select ggircs_portal.init_application_administration_form_result(
  1, (
